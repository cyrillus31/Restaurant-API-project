...
- [Описание API](#описание-api)
    ...
    - [Реализована синхронизация с таблицей администратора в Google sheets](#реализована-синхронизация-с-таблицей-администратора-в-google-sheets)
...
Файл [.env.example](/.env.example) используется для хранения переменных окружения, необходимых для подключения к БД внутри Docker контейнера.
...
- **Проект *и тесты* покрыты тайпхинтами и проходят все pre-commit хуки**
...
> **Important**
> **Файл [Menu.xlsx](admin/Menu.xlsx) связан с контейнером напрямую через [Bind mount](https://docs.docker.com/storage/bind-mounts/) и поэтому доступен администратору для редактирования напрямую через файловую систему пользовательского компьютера.**
...
### Добавлен query string параметр `id` для http методов POST
Для первоначального заполнения таблицы и последующего добавления элементов из Excel файла (который содержит заданные пользователм id) были доработаны существующие POST эндпоинты: добавлена поддержка [Query String](https://en.wikipedia.org/wiki/Query_string) с опциональным параметром *id* (например `POST http://127.0.0.1:8000/menus/?id=123`), что позволяет передавать используемый пользователем в Excel таблице ID объекта в базу данных для создании в ней этого объека с пользовательским ID. В случае, если параметр не будет передан в URL, то в качестве первичного ключа будет сгенерирован UUID.

<br><br>

## Реализована синхронизация с таблицей администратора в Google sheets

Синхронизация с таблицей Google sheets является альтернативой предыдущей опции и будет работать только при запуске через соответствующий Docker-compose файл. О командах для запуска разных версий API можно прочтить в одном из разделов ниже.
...
> **Important**
> С момента запуска первого контейнера и до момента заполнения БД из Excel файла (или из Google sheets) может требоваться окло 30 секунд.
...
> **Note**
> Тесты на Postman необходимо запускать при пустой БД, для этого следует очистить [Excel](admin/Menu.xlsx) таблицу администратора и подождать ~15 секунд, чтобы база данных пришла в соответствие.
...
> **Warning**
> В случае некорректной работы API и контейнеров: перед запуском каждой команды Docker-compose рекомендуется удалять ранее собранные контейнеры и образы.
