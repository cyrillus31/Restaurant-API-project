...
class MenuRepository:
    ...
    async def add(self, menu: schemas.MenuCreate | schemas.SubmenuCreate | schemas.DishCreate, **kwargs) -> models.Menu | models.Submenu | models.Dish | None | Any:
        # create query
        lookup_query = self.db_model.select().where(self.db.c.title == menu.title)
        # run query
        menu_exists = await self.db.fetch_one(query=lookup_query)
        ...
        create_query = self.db_model.isnert().valus(**menu.dict())
        ...
        new_menu = await self.db.fetch_one(query=lookup_query)
        # return {new_menu}
