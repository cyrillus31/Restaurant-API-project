...
# Create testing
async def test_create_menu(session: AsyncSession, client: AsyncClient, PREFIX: str) -> None:
    create_data = {
...
# Read testing
async def test_get_menu(session: AsyncSession, client: AsyncClient, PREFIX: str, test_menus: list[models.Menu]) -> None:
...
async def test_get_menu_not_exists(session: AsyncSession, client: AsyncClient, PREFIX: str, test_menus: list[models.Menu]) -> None:
...
async def test_read_menus(session: AsyncSession, client: AsyncClient, PREFIX: str, test_menus: list[models.Menu]) -> None:
...
async def test_read_menus_empty(session: AsyncSession, client: AsyncClient, PREFIX: str) -> None:
    res = await client.get(f'{PREFIX}/menus/')
    assert res.status_code == 200
    assert res.json() == []
...
async def test_update_menu(session: AsyncSession, client: AsyncClient, PREFIX: str, test_menus: list[models.Menu]) -> None:
    test_menu_id = test_menus[0].id
    update_data = {
...
async def test_update_menu_not_exists(session: AsyncSession, client: AsyncClient, PREFIX: str, test_menus: list[models.Menu]) -> None:
    update_data = {
        'title': 'UPDATED test menu 1 title',
        'description': 'UPDATED test menu 1 description',
    }
...
async def test_delete_menu(session: AsyncSession, client: AsyncClient, PREFIX: str, test_menus: list[models.Menu]) -> None:
    test_menu_id = test_menus[0].id
